...
// Cache to avoid re-compositing the same thumbnail within one batch
...
/**
 * Extract a thumbnail frame from a video buffer using sharp.
 * For uploaded video files, we extract the first frame.
...
export async function extractVideoThumbnail(
    ...
    // sharp cannot decode video natively — for .mp4/.mov we return null
    // and let the caller handle custom thumbnail upload.
    // This is a placeholder for future ffmpeg integration.
    try {
        // If the file happens to be a GIF or animated webp, sharp can handle it
        const metadata = await sharp(videoBuffer).metadata()
        if (metadata.width && metadata.height) {
            return await sharp(videoBuffer)
                .resize(640, 360, { fit: "cover" })
                .jpeg({ quality: 80 })
                .toBuffer()
        }
    } catch {
        // Not an image-like format — expected for most video files
    }
    return null
}
 ...
 */
export async function compositePlayButton(
    thumbnailUrl: string
): Promise<string> {
    // Check cache first
    ...
    if (!response.ok) {
        console.error(
            `Failed to fetch thumbnail: ${thumbnailUrl} (${response.status})`
        )
        return thumbnailUrl
    }
    ...
    const playButtonPath = path.join(
        process.cwd(),
        "public",
        "play-button-overlay.png"
    )
    ...
    // 7. Hash-based upload for deduplication
    const hashHex = createHash("sha256").update(composited).digest("hex")
    const storagePath = `video-thumbnails/${hashHex}.png`
 ...
 */
export async function addPlayButtonsToVideoThumbnails(
    html: string
): Promise<string> {
    ...
    while ((match = linkImagePattern.exec(html)) !== null) {
        ...
        if (
            isVideoUrl(linkUrl) &&
            imgSrc &&
            !imgSrc.includes("video-thumbnails/")
        ) {
